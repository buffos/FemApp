...
void FEM::ArcLengthMethod()
{
	...
	ColumnVector Temp=0;
	ColumnVector DU=0;
	ColumnVector LTemp=0;ColumnVector UTemp=0;
	...
	for(int l=CurrentStage;l<=NumberOfStages;l++)
	{
		...
		for(int incr=1;incr<=fem->IterationsDesired;incr++)
		{
			...
			fem->AssembleInternalLoads(true,fem->Method);
			K= this->AssembleStiffnessMatrix();KTemp=K;
			ldl(K,Lower,Diagonal,neg,VectorLength);
			fem->ConstrainCenter->ConstrainMatrix(K,CurrentStage);			//Impose Boundary Conditions
			Residual=DT;
			if (SolverType==1) { Kinv=K.i();}
			switch (SolverType)
			{
			case 1 : //Gauss Elimination
				LTemp=LCS;	fem->ConstrainCenter->ConstrainVector(LTemp,CurrentStage);
				DT=(Kinv*LCS).Rows(1,ActiveSystemDofs);
				break;
			case 2:
